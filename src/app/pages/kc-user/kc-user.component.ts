...
import { DropdownSelector, MultiSelectSelector, FormGroupColumn, TableColumn } from '../../models';
...
export class KcUserComponent implements OnInit, OnDestroy {
    ...
    constructor(
        ...
        this.formCols = [
            ...
            { field: 'structure', header: 'Direction ou service ', type: 'dropdown', visible: true, required: true },
    ...
    loadRoles() {
        this.authService
            ...
            .subscribe({
                next: (resp) => {
                    const fetchedRoles = (resp.body || []).map(r => ({ value: r.name, label: r.name }));
    ...
    loadStuctures() {
            this.structureService
                ...
                .subscribe({
                    next: (resp) => {
                        this.structures = (resp.body || []).map(value => ({
                            value: value.id,
