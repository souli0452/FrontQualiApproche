...
describe('WorkflowEditorComponent — attribution des codes d\'étape', () => {
  ...
  it('distingue deux étapes issues du même modèle, destinations comprises', () => {
    ...
    const approbation = etapes[0].transitions?.find((t: any) => t.decision === 'APPROUVE');
  ...
  it('retient toutes les actions d\'une étape, y compris de même nature', () => {
    ...
    expect(actions.map((a: any) => a.code)).toEqual(['APPROUVE', 'REJETE', 'DEMANDER_COMPLEMENT']);
    expect(actions.filter((a: any) => a.decision === 'APPROUVE').length).toBe(2);
  });
